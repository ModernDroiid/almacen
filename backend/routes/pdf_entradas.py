# ============================================================
# pdf_entradas.py — Genera el PDF de entrada al estilo
...
pdf_entradas_bp = Blueprint('pdf_entradas', __name__)

@pdf_entradas_bp.route('/entrada/<int:id>', methods=['GET'])
def generar_pdf_entrada(id):
    ...
    entrada = dict(cursor.fetchone())

    # Traemos el detalle con modelo, marca, serial
    cursor.execute('''
    ...
    # Título y número en una columna, centrados
    bloque_titulo = [
        Paragraph('ENTRADA ALMACEN', ParagraphStyle(
            'tit', fontSize=16, fontName='Helvetica-Bold',
            textColor=colors.HexColor('#0d2137'), alignment=TA_CENTER
        )),
        Spacer(1, 0.2*cm),
        Paragraph(f'N° {entrada.get("numero_documento", "")}',
    ...
    # ── Info de la entrada ────────────────────────────────────
    fecha_str = entrada.get('fecha', '')[:10] if entrada.get('fecha') else ''

    info_data = [
        [
            Paragraph('De la seccion:', estilo_label),
            Paragraph(entrada.get('obra', ''), estilo_valor),
            Paragraph('Fecha:', estilo_label),
            Paragraph(fecha_str, estilo_valor),
        ],
        [
            Paragraph('Con destino a:', estilo_label),
            Paragraph(entrada.get('destino', 'Almacen'), estilo_valor),
            Paragraph('Observaciones:', estilo_label),
            Paragraph(entrada.get('observaciones', ''), estilo_valor),
        ],
    ]
    ...
    # Filas vacías para que quede espacio como el formato original
    ...
    return send_file(
        ...
        as_attachment=False,
        download_name=f'entrada_{entrada.get("numero_documento", id)}.pdf'
    )